...
Instead, use the two-pointer approach, which is a common
 technique in which we initialize two variables (also called a pointer in this context)
...
one variable to point at the beginning of a list and a second variable/pointer
to point at the end of list. We then shift the pointers to move inwards through 
...
def reverse_list(lst):
    ...
    # iterte through the list with the two pointers
    while start < end:
