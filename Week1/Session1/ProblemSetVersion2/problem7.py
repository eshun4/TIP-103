"""
Write a function diagonal_sum() that accepts a 2D n x n matrix grid 
and returns the sum of the matrix diagonals. Only include the sum of 
all the elements on the primary diagonal and all the elements in the
...
def diagonal_sum(grid):
	primary_diag, sec_diag = 0, 0
	for i in range(len(grid)):
		primary_diag += grid[i][i]
                
	for index in range(len(grid)):
		sec_diag += grid[index][len(grid) - 1 - index]

	total_diag = primary_diag + sec_diag
	if len(grid) % 2 == 1:
		total_diag -= grid[len(grid) // 2][len(grid) // 2]
	return total_diag
